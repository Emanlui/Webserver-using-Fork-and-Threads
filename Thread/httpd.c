...
void *connect_to_server(void* slot){
  
        int *pointer_slot = (int *) slot;
        int real_slot = *pointer_slot;
        close(listenfd);
        respond(real_slot);
        printf("MAX LIMIT: %d\n", max_limit_of_clients);
        close(clients[real_slot]);
        exit(0);
...
void serve_forever(const char *PORT, int max_connections) {
  ...
  array_of_threads = mmap(NULL, sizeof(*array_of_threads) * max_connections,
                 PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
